...
import org.jakstab.loader.pe.BinaryInputBuffer;
...
public class ELFModule implements ExecutableImage {
	...
	public ELFModule(File moduleFile, Architecture architecture) throws IOException, BinaryParseException {
		...
		BinaryInputBuffer inBuf = new BinaryInputBuffer(new FileInputStream(moduleFile));
		data = inBuf.getByteArray();
		...
		X86Disassembler disasm = new X86Disassembler(data);
		// push GOT + 4
	...
	/*
	 * @see org.jakstab.loader.ExecutableImage#getByteArray()
	 */
	@Override
	...
	public Disassembler getDisassembler() {
		if (disassembler == null) {
			disassembler = new X86Disassembler(this);
		}
		return disassembler;
	}
	...
	protected byte getByteAt(long fp) {
		return data[(int)fp];
	}
	...
	public RTLNumber readMemoryLocation(RTLMemoryLocation m) {
		...
		if (fp > 0) {
			...
			// Check if reference lies within file area
			if (fp + bytes >= data.length) {
				return null;
			}
			
			// OR together the least significant bytes 
			for (int i=0; i<bytes - 1; i++) {
				val = val | ((long)(0xFF & getByteAt(fp + i))) << (i*8);
			}
			// do not mask the MSB with 0xFF, so we get sign extension for free
			val = val | (((long)getByteAt(fp + bytes - 1)) << (bytes - 1) * 8);
