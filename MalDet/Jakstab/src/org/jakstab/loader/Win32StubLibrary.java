	...
	private AbsoluteAddress createStubInstance(String library, String function) {
		...
		if (library.equals(jakstab_internal)) {
			...
			if (function.equals("_jakstab_print_driver_object@4")) {
				...
				int mjFunArray = 0x38;
				seq.addLast(new RTLAssignment(32, driverObject, factory.createMemoryLocation(
			...
			if (library.toUpperCase().startsWith("KERNEL32") && function.equals("GetProcAddress")) {
				...
				seq.addLast(new RTLAssignment(32, factory.createVariable("%eax"), loadExpression));
			} else if (library.toUpperCase().startsWith("KERNEL32") && function.startsWith("GetModuleHandle")) { 
				seq.addLast(new RTLAssignment(32, factory.createVariable("%eax"), arg0));
			} else {
				// overwrite registers according to ABI
				seq.addLast(new RTLAssignment(32, factory.createVariable("%eax"), factory.nondet(32)));
			}
			seq.addLast(new RTLAssignment(32, factory.createVariable("%ecx"), factory.nondet(32)));
			seq.addLast(new RTLAssignment(32, factory.createVariable("%edx"), factory.nondet(32)));
		}

		// store return address in retaddr
		if (returns) {
			seq.addLast(new RTLAssignment(32, Program.getProgram().getArchitecture().returnAddressVariable(), 
		...
		seq.addLast(new RTLAssignment(arch.stackPointer().getBitWidth(), 
				arch.stackPointer(), 
				factory.createPlus( 
						arch.stackPointer(), 
