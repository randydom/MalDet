 ...
 * Copyright 2008-2010 Johannes Kinder <kinder@cs.tu-darmstadt.de>
 *
...
public final class SubstitutionState implements AbstractState {
	...
	public AbstractState abstractPost(StateTransformer transformer, Precision precision) {
		...
		return statement.accept(new DefaultStatementVisitor<SubstitutionState>() {
			...
			@Override
			public SubstitutionState visit(RTLAssignment stmt) {
				...
				if (lhs instanceof RTLMemoryLocation) {
					...
					SubstitutionElement aAddress = abstractEval(m.getAddress());
					if (!aAddress.isTop())
						lhs = factory.createMemoryLocation(m.getSegmentRegister(), aAddress.getExpression(), m.getBitWidth());
				}

				// Remove existing substitution for the LHS
				post.aVarVal.remove(lhs);
				// If RHS is a pure variable or memory expression, 
				// assign RHS to LHS as substitution
				...
				// If LHS is a memory location, remove all substitutions that might alias with it
