	...
	public Set<AbstractState> abstractPost(final RTLStatement statement, final Precision precision) {
		...
		return statement.accept(new DefaultStatementVisitor<Set<AbstractState>>() {
			...
			public Set<AbstractState> visit(RTLAssignment stmt) {
				...
				// only store variables and constant memory addresses
				if (lhs instanceof RTLVariable) {
					...
					//if (evaledRhs.isTop())
					//	logger.warn("Setting " + lhs + " to top! " + BasedNumberValuation.this);
					
					post.setValue((RTLVariable)lhs, evaledRhs, eprec);
			...
			public Set<AbstractState> visit(RTLAlloc stmt) {
				...
				MemoryRegion newRegion = MemoryRegion.create("alloc" + stmt.getLabel() + 
