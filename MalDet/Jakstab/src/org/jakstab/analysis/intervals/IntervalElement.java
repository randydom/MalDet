 ...
 */
public class IntervalElement implements AbstractValue, BitVectorType, Iterable<Long> {
	...
	public Set<RTLNumber> concretize() {
		...
		if (getRegion() != MemoryRegion.GLOBAL || size() > 100) {
			return Collections.singleton(null); // Full set
		}
	 ...
	 */
	public IntervalElement plus(IntervalElement op) {
	...
	public IntervalElement negation() {
	 ...
	 */
	public IntervalElement multiply(IntervalElement op) {
		MemoryRegion newRegion = region.join(op.region);
