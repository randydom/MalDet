...
public class Architecture {
	...
	public StatementSequence getRTLEquivalent(AbsoluteAddress address, Instruction instr) {
		...
		// evaluate again, to use infered bitwidths.
	...
	public void parseSSL(String fileName) throws FileNotFoundException, ANTLRException {
		...
		for (Iterator<Map.Entry<String, SSLFunction>> iterator = instrPrototypes.entrySet().iterator(); iterator.hasNext();) {
			...
			if (rtlBody != null) {
				...
				if (exitStatement instanceof RTLAssignment) {
					RTLAssignment assignment = (RTLAssignment)exitStatement;
				...
				// Remove flags
				StatementSequence newSeq = new StatementSequence();
				copyLoop: for (RTLStatement s : rtlBody) {
					if (s instanceof RTLAssignment) {
						RTLAssignment a = (RTLAssignment)s;
						for (int i=0; i<getStatusFlags().length; i++) {
							if (getStatusFlags()[i].equals(a.getLeftHandSide())) {
								continue copyLoop;
							}
						}
					}
